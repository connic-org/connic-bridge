...
def test_cli_starts_in_reject_all_mode_when_no_allowed_hosts_are_configured(monkeypatch, capsys):
    calls = []
    ...
    output = capsys.readouterr().out
    assert "Warning: No allowed hosts specified. The bridge will reject all connections." in output
    assert "Use --allow host:port or set ALLOWED_HOSTS env var." in output
    assert f"Relay:         {cli.DEFAULT_RELAY_URL}" in output
    assert "Allowed hosts: (none)" in output
